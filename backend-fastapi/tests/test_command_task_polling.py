...
覆盖 wait_task_result 的响应解读（interpret_task_poll）与轮询循环行为：
...
class TestInterpretTaskPoll:
    def test_running_status_continues(self):
        action, result = interpret_task_poll(
    ...
    def test_accepted_and_cancelling_continue(self):
        for status in ("accepted", "pending", "cancelling"):
            action, _ = interpret_task_poll(200, {"status": status})
            assert action == "running"
    ...
    def test_unknown_status_fails_immediately(self):
        ...
        action, result = interpret_task_poll(200, {"status": "weird"})
        assert action == "done"
        assert result["success"] is False
    ...
    def test_404_is_terminal(self):
        ...
        action, result = interpret_task_poll(404, None)
        assert action == "gone"
        assert result["success"] is False
        assert "任务不存在" in result["stderr"]

    def test_5xx_is_retryable(self):
        action, result = interpret_task_poll(502, None)
        assert action == "retry"
        assert result is None

    def test_non_json_200_is_retryable(self):
        """200 但 JSON 解析失败（payload=None）按暂时性失败处理"""
        action, result = interpret_task_poll(200, None)
        assert action == "retry"
...
async def test_wait_task_result_fails_fast_on_connection_errors(monkeypatch):
    ...
    monkeypatch.setattr(worker_client_module.httpx, "AsyncClient", _AlwaysFailClient)
    monkeypatch.setattr(worker_client_module.asyncio, "sleep", _no_sleep)

    start = time.monotonic()
    result = await wait_task_result("10.0.0.1", "8080", "task-1")
...
async def test_wait_task_result_404_returns_immediately(monkeypatch):
    ...
    monkeypatch.setattr(worker_client_module.httpx, "AsyncClient", _NotFoundClient)
    monkeypatch.setattr(worker_client_module.asyncio, "sleep", _no_sleep)

    result = await wait_task_result("10.0.0.1", "8080", "task-gone")
