...
<ElTableColumn label="操作" min-width="140">
  <template #default="{ row }">
    ...
    <a class="env-link" @click="handleUpgrade(row)" 
       v-if="row.status === 'online' && needUpgrade(row.version)">升级</a>
...
```typescript
import { batchUpgradeApi } from '#/api/core/env-machine-upgrade';

// 判断是否需要升级（需要获取目标版本，这里简化处理）
function needUpgrade(version: string | undefined): boolean {
  // 实际应对比配置中的版本
  return !!version && version < '20260410150000';
}
